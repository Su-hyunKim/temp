...
<body>
<h2>** Spring Oracle Product PageList **</h2>
<br>
...
<div align="center">
	...
	<c:choose>
		<c:when test="${sPageNo>pageNoCount}">
			<a href="ppagelist?currPage=1">FF</a>&nbsp;
			<a href="ppagelist?currPage=${sPageNo-1}">&lt;</a>&nbsp;&nbsp;
		</c:when>
		<c:otherwise>
	...
	<c:forEach var="i" begin="${sPageNo}" end="${ePageNo}">
		...
		<c:if test="${i!=currPage}">
			<a href="ppagelist?currPage=${i}">${i}</a>&nbsp;
		</c:if>
	</c:forEach>
	&nbsp;
	<c:choose>
		<c:when test="${ePageNo<totalPageNo}">
			<a href="ppagelist?currPage=${ePageNo+1}">&gt;</a>&nbsp;
			<a href="ppagelist?currPage=${totalPageNo}">LL</a>
		</c:when>
		<c:otherwise>
